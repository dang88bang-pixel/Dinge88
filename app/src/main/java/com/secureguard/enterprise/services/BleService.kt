 ...
 * a short (2 s) hardware-filtered scan is run; a hit is only reported when the
 * asset's MAC address is actually seen over the air. If BLE is unavailable,
 * permission is missing or nothing is seen, `null` is returned — no simulated
 * fallback.
 */
@Singleton
class BleService @Inject constructor(
    ...
    suspend fun searchAsset(asset: Asset): Detection? {
        if (!hasScanPermission() || bluetoothAdapter?.isEnabled != true) return null
