...
// ---- Authentifizierung & Nutzungseinschränkungen ----
// Standard: KEINE Nutzungseinschränkungen. Die App erzeugt ihre Zugangsdaten
// selbst (siehe MqttCredentialManager in der App) – der Broker akzeptiert
// app-erzeugte Benutzer/Passwörter ohne Vorgaben und ohne Topic-ACLs.
//
// Optional strikt: SG_MQTT_RESTRICTED=true + SG_MQTT_USERNAME/SG_MQTT_PASSWORD
// setzen – dann werden nur diese Zugangsdaten akzeptiert (z. B. in Docker).
const SG_RESTRICTED = String(process.env.SG_MQTT_RESTRICTED || '').toLowerCase() === 'true';
const SG_USER = process.env.SG_MQTT_USERNAME || '';
const SG_PASS = process.env.SG_MQTT_PASSWORD || '';

if (SG_RESTRICTED && SG_USER && SG_PASS) {
  ...
  console.log(`[broker] STRIKT: nur Benutzer '${SG_USER}' zugelassen`);
} else {
  // Keine Nutzungseinschränkungen: app-erzeugte Zugangsdaten willkommen,
  // keine Topic-ACLs, keine Rate-Limits.
  aedes.authenticate = (client, username, password, done) => {
    console.log(
      `[broker] Verbindung ohne Einschränkung: ${username || '(ohne Benutzer)'}` +
      (password && String(password).length ? ` · Passwort (${String(password).length} Zeichen, in App erzeugt)` : ' · ohne Passwort')
    );
    done(null, true);
  };
  console.log('[broker] KEINE Nutzungseinschränkungen – app-erzeugte Zugangsdaten akzeptiert');
}
